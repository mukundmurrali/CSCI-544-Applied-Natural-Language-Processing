...
def FirstLayer(net, l2_reg_val, is_training):
    ...
    net = tf.contrib.layers.fully_connected(
            net, 40, activation_fn=None, weights_regularizer=None,biases_initializer=None, normalizer_fn=normalization_function,normalizer_params=normalizer_params)
    #net = tf.contrib.layers.batch_norm(net,is_training=is_training)
    losses = l2_reg_val * tf.reduce_sum(tf.math.square(net))
    tf.losses.add_loss(losses, loss_collection=tf.GraphKeys.REGULARIZATION_LOSSES);
    net = tf.nn.tanh(net)
...
def EmbeddingL2RegularizationUpdate(embedding_variable, net_input, learn_rate, l2_reg_val):
    ...
    l2norx = tf.nn.l2_normalize(net_input, axis=1);
    xTx = tf.linalg.matmul(tf.transpose(l2norx), l2norx)
    xTxY = tf.linalg.matmul(xTx, embedding_variable)
    result = 2 * learn_rate * l2_reg_val * xTxY
    return tf.math.subtract(embedding_variable, result)
...
def EmbeddingL1RegularizationUpdate(embedding_variable, net_input, learn_rate, l1_reg_val):
    ...
    #l2normxT = tf.nn.l2_normalize(tf.transpose(net_input), axis=1);
    l2normx = tf.nn.l2_normalize(net_input, axis=1);
    xTy = tf.math.sign((tf.linalg.matmul(l2normx, embedding_variable)));
    result = learn_rate * l1_reg_val * (tf.linalg.matmul(tf.transpose(l2normx),xTy));
    return tf.math.subtract(embedding_variable, result)
...
def SparseDropout(slice_x, keep_prob=0.5):
    ...
    slice_x[i[positions], j[positions]] = 0; #make those random positions as zero
    return slice_x
...
def ComputeTSNE(embedding_matrix):
    ...
    #n_components : Dimension of the embedded space - For our case this is 2 for our case
    #outout is the embedding matrix of 
...
def VisualizeTSNE(sess):
    ...
    #computer the embeddings using the TSNE..
    ...
    #this variable is used to hold the word embeddings
    words_embeddings = {}
    
    #iterate the (terms,index) dictionary to form the 
    #word_embeddings with the following structure 
    #(word, (x-axis, y -axis))
    for w,idx in TERM_INDEX.items():
        words_embeddings[w] = tsne_embeddings[idx];


    # display scatter plot
    #this creatates a representation to be shown..
    f = plt.figure()
    for classes,words in class_to_words.items():
        #for each class we get the embeddigs and plot it in the scatter plot..
        if(classes == 'positive'):
            for label in words :
                if(classes == 'positive'):
                        for label in words :
                            #get the x and y axis for the label from the embeddings.
                            index = words_embeddings[label]; 
                            x = index[0];
                            y = index[1];
                            #plot the point into the graph..
                            plt.scatter(x, y, c = 'blue')
                            #plot the label name into the point with some delta xytext
                            plt.annotate(label, xy=(x, y), xytext=(0.001, 0.001), textcoords='offset points')
        if(classes == 'location'):
                        for label in words :
                            index = words_embeddings[label];
                            x = index[0];
        ...
        if(classes == 'negative'):
                        for label in words :
                            ...
                            plt.annotate(label, xy=(x, y), xytext=(0.001, 0.001), textcoords='offset points')
    #save the plot into the the specified pdf dile.. 
...
def BuildInferenceNetwork(x, l2_reg_val, is_training):
    ...
    EMBEDDING_VAR = [variable for variable in tf.trainable_variables() if variable.name.endswith('weights:0')][0];
